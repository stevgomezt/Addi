...
def botones_descarga(Xf = None , variable = 'RangoConsumo', col = None ):#,cols = [col11, col12, col13, col14,col15]}
                        
    for categoria in Xf[variable].unique(): 

        download_excel_torta(Xf[Xf[variable]==categoria] ,nombre=categoria, col=col)
                            ...
                            if mayus==True:
                                if total == False:
                                    ...
                                    bar = alt.Chart(df_group).mark_bar().encode(
                                        x=alt.X('Cantidad', axis=alt.Axis(ticks=True,title=config['x_axis_title'])),
                                        y=alt.Y(config['y_axis'] + ":N", sort=list(df_group[config['y_axis']]),axis=alt.Axis(ticks=False, title='')),#
                                    ...
                                    bar = alt.Chart(df_group).mark_bar().encode(
                                        x=alt.X('Cantidad', axis=alt.Axis(ticks=False,title=config['x_axis_title'])),
                                        y=alt.Y(config['y_axis'] + ":N", sort=list(df_group[config['y_axis']]),axis=alt.Axis(ticks=False, title='')),#
                                        tooltip=[config['y_axis']+":N",'Cantidad:Q', 'Porcentaje:O'],
                                        text=alt.Text('Porcentaje:N')
                                ...
                                bar = alt.Chart(df_group).mark_bar().encode(
                                    x=alt.X('Cantidad', axis=alt.Axis(ticks=False,title=config['x_axis_title'])),
                                    # y=config['y_axis'] + ":N",
                                    y=alt.Y(config['y_axis'] + ":N", sort=list(df_group[config['y_axis']]),axis=alt.Axis(ticks=False, title=None)),#, title=None
                                    tooltip=[config['y_axis']+":N",'Cantidad:Q', 'Porcentaje:O'],
                                    text=alt.Text('Porcentaje:N')
...
def scatter_plot(df,col=None): 
    ...
    fig = px.scatter(df,x='DEPARTAMENTO', y='ACTIVIDADES', 
                        ...
                        color_continuous_scale=colorscale#'Plasma'#px.colors.sequential.Cividis#'Plotly3'#'matter_r'#'purples_r'
                        )
                ...
                if final_flag == False:
                    ...
                    for i,j in zip(range(len(logs)),logs_riesgo):
                        ...
                        if i ==0:       # Si es el primer log agrega '¡Ups! Parece que hay un problema.'
                            ...
                            if j =='1':
                                st.write('<div align="center"><h2>¡Ups! Parece que hay un problema.</h2></div>', unsafe_allow_html=True)
...
#-----------------------------------------------------13/06/23
        ...
        with vista1:    # Modelo Multiples Clientes
            try:              
                ...
                keys =  ['SINCATALOGAR','MENORA5000', 'ENTRE5000Y10000', 'ENTRE10000Y55000',  'MAYORA55000']
                values = ['Sin catalogar','Menor a 5000 kW⋅h',  'Entre 5000 y 10000 kW⋅h','Entre 10000 y 55000 kW⋅h']
                ...
                container0.markdown(f'<div class="custom-container"></div>', unsafe_allow_html=True)
                ...
                col1_container1,col2_container1, col3_container1 = container1.columns(spec=[2.5,2.3,1])
                # Dividir el segundo contenedor en tres columnas
                col1_container2, col2_container2= container2.columns(2) # , col3_container2, col4_container2 
...
#-----------------------------------------------------
                ...
                container1.markdown(f'<div class="custom-container"></div>', unsafe_allow_html=True)
                
                col1_container1.markdown(f'<h1 style="text-align: center; font-size: {tamaño1}px; color: {color1};">{texto1}</h1>', unsafe_allow_html=True)
                
                col1_container1.markdown(f'<h1 style="text-align: center; font-size: {tamaño2}px; color: {color2}">{texto2}</h1>', unsafe_allow_html=True)
                ...
                generar_graficos(Xf,auto_orden=True, configuraciones=configuraciones,color =0,total=False) 
